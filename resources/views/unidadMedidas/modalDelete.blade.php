<!-- Modal de Confirmación para Eliminar Unidades de Medida -->
<div x-show="deleteOpen" x-transition
    ...
    <div @click.outside="deleteOpen = false" class="bg-white rounded-lg shadow-lg p-4 w-full max-w-md mx-4">
        ...
        <p class="mb-4">¿Seguro que deseas eliminar la unidad de medida:
            <strong>{{ $unidadMedida->descripcion }}</strong>?
        </p>
        <div class="flex justify-end gap-2">
            <button type="button" @click="deleteOpen = false"
                class="px-4 py-2 bg-gray-300 rounded hover:bg-gray-400">Cancelar</button>
            <form action="{{ route('unidadMedidas.destroy', $unidadMedida->id) }}" method="POST" class="inline">
                @csrf
                @method('DELETE')
                <button type="submit"
