...
<div x-show="editOpen" x-transition
    ...
    <div @click.outside="editOpen = false" class="bg-white rounded-lg shadow-lg p-4 w-full max-w-2xl relative">
        <h2 class="text-xl font-bold mb-4">Editar Tipo de Maquinaria
        </h2>
        <form action="{{ route('tipomaquinarias.update', $tipomaquinaria) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-4">
                <label class="block font-medium">Descripción</label>
                <input type="text" name="descripcion" value="{{ old('descripcion', $tipomaquinaria->descripcion) }}"
